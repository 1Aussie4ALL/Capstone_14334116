...
MODEL_PATH = 'mri_detection_classifier_best.h5'
...
def load_mri_detection_model():
    ...
    try:
        if os.path.exists(MODEL_PATH):
            ...
            print(f"✅ MRI Detection Model (Best) loaded successfully from: {MODEL_PATH}")
            return True
        else:
...
def check_model():
    ...
    if model is not None:
        return jsonify({
            'status': 'success',
            'message': 'MRI Detection Model (Best) loaded successfully',
...
if __name__ == '__main__':
    print("🚀 Starting MRI Detection Web App (V2 - Best Model)...")
    ...
    if load_mri_detection_model():
        ...
        print("💡 Or check if the model file exists: mri_detection_classifier_best.h5")
