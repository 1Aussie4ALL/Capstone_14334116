...
print("🔍 Comprehensive VariationA MRI Classifier Evaluation")
print("=" * 60)
print("📊 This classifier combines original dataset + Variation A 800 dataset")
print("=" * 60)

# Configuration
IMG_SIZE = 128
MODEL_PATH = '../../VariationA_Enchanced/Models/mri_variationA_classifier.h5'
...
def load_model_and_predict():
    ...
    global MODEL_PATH
    print(f"\n🤖 Loading VariationA model: {MODEL_PATH}")
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        alternative_paths = [
            ...
            '../Models/mri_variationA_classifier_final.h5',
...
def plot_confusion_matrix(y_true, y_pred, save_path, title):
    ...
    plt.title(f'{title} - VariationA Classifier (Normalized)', fontsize=16, fontweight='bold', pad=20)
...
def plot_precision_recall_curves(probabilities, y_true, save_path):
    ...
    for i, class_name in enumerate(ALL_CLASSES):
        ...
        axes[i].set_title(f'{class_name} - PR Curve (VariationA)', fontsize=14, fontweight='bold')
    ...
    plt.suptitle('VariationA Classifier - Precision-Recall Curves', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_roc_curves(probabilities, y_true, save_path):
    ...
    for i, class_name in enumerate(ALL_CLASSES):
        ...
        axes[i].set_title(f'{class_name} - ROC Curve (VariationA)', fontsize=14, fontweight='bold')
        axes[i].legend(fontsize=10)
        axes[i].grid(True, alpha=0.3)
        axes[i].set_xlim([0, 1])
        axes[i].set_ylim([0, 1])
    ...
    plt.suptitle('VariationA Classifier - ROC Curves', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_calibration_curves(probabilities, y_true, save_path):
    ...
    for i, class_name in enumerate(ALL_CLASSES):
        ...
        axes[i].set_title(f'{class_name} - Calibration (VariationA)\nECE={ece:.3f}, Brier={brier:.3f}', 
    ...
    plt.suptitle('VariationA Classifier - Calibration Curves', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    try:
        ...
        print(f"  Note: VariationA classifier was trained on original + Variation A 800 dataset")
        ...
        print("\n" + "="*50)
        print("📊 TRAINING/VALIDATION SET EVALUATION (VariationA)")
        print("="*50)
        ...
        # Evaluate Test set
        print("\n" + "="*50)
        print("📊 TEST SET EVALUATION (VariationA)")
        print("="*50)
        ...
        print("\n🎉 VariationA Classifier evaluation completed successfully!")
        print(f"📁 Results saved in: Evaluation_Results_VariationA/")
        print(f"🔍 Compare with original 2Layer classifier results for analysis")
