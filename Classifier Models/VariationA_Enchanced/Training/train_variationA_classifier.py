...
print("🚀 Starting VariationA Classifier Training...")
print("=" * 60)
...
VARIATION_A_PATH = '../../Photometric Augmentation - Variation A/data_used_for_variation_A_800'
MODEL_SAVE_PATH = '../Models/mri_variationA_classifier.h5'
...
print(f"📁 Variation A dataset path: {VARIATION_A_PATH}")
...
def load_and_preprocess_data():
    ...
    # Load Variation A dataset
    print("  🔄 Loading Variation A dataset...")
    for mri_class in MRI_CLASSES:
        class_path = os.path.join(VARIATION_A_PATH, mri_class)
        if os.path.exists(class_path):
            print(f"    Loading {mri_class} (Variation A): ", end="")
            count = 0
            for filename in os.listdir(class_path):
                if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                    img_path = os.path.join(class_path, filename)
    ...
    not_mri_path = os.path.join(VARIATION_A_PATH, 'not_mri')
    if os.path.exists(not_mri_path):
        print(f"    Loading not_mri (Variation A): ", end="")
        count = 0
        for filename in os.listdir(not_mri_path):
            if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                img_path = os.path.join(not_mri_path, filename)
...
def evaluate_model(model, X_val, y_val, label_to_index):
    ...
    plt.title('Confusion Matrix - VariationA Classifier')
...
def main():
    ...
    try:
        ...
        print(f"\n💾 Model saved as: mri_variationA_classifier_final.h5")
        ...
        plt.title('VariationA Model Accuracy')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.legend()
        ...
        plt.title('VariationA Model Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        ...
        print("\n🎉 VariationA training completed successfully!")
        print(f"📊 Final accuracy: {accuracy*100:.2f}%")
        print(f"💾 Best model saved: {MODEL_SAVE_PATH}")
        print(f"📈 Training plots saved: training_history_variationA.png")
        print(f"📊 Confusion matrix saved: confusion_matrix_variationA.png")
